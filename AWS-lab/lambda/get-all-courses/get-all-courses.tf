...
resource "aws_iam_role_policy" "get-all-courses-lambda-policy" {
  name = "${var.name}-lambda-policy"
  role = aws_iam_role.get-all-courses-lambda-role.id
...
data "archive_file" "get-all-courses-lambda" {
  type        = "zip"
  source_file = "${path.module}/${var.name}-lambda.js"
  output_path = "${path.module}/${var.name}-lambda.zip"
}

resource "aws_lambda_function" "get-all-courses-lambda" {
